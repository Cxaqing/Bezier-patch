// Revolution.cpp: implementation of the CRevolution class.
//
//////////////////////////////////////////////////////////////////////
...
void CRevolution::ReadCubicBezierControlPoint(CP3* ctrP)//三次Bezier曲线4个控制点初始化
{
	for (int i = 0; i < 4; i++)
		P[i] = ctrP[i];
	//读入回转体的数据结构
...
void CRevolution::ReadVertex()//读入回转体的所有控制点
{
	const double m = 0.5523;//魔术常数
	//回转一圈需要4个面,每面16个点,共64个点
	//第一块面片
...
void CRevolution::ReadPatch()//曲面片表
{
	//第1卦限面片
	S[0].pNumber=16;
...
void CRevolution::DrawRevolutionSurface(CDC* pDC)//绘制回转体曲面
{
	for(int nPatch=0;nPatch<4;nPatch++)
	{
	    ...
	    surf.ReadControlPoint(P3);//读入控制点
		CPen NewPen,*pOldPen;
		NewPen.CreatePen(0,2,RGB(135,206,250));              //原来150,150,255
		pOldPen=pDC->SelectObject(&NewPen);
        surf.DrawCurvedPatch(pDC);//画双三次Bezier曲线
		pDC->SelectObject(pOldPen);
		NewPen.DeleteObject();
		//surf.DrawControlGrid(pDC);//画绘制网格
	}
}
